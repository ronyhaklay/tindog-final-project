...
export function MatchCelebration({ requestId, dogName, photoPath, barkAudioPath, shelterName }: { requestId: string; dogName: string; photoPath: string | null; barkAudioPath: string | null; shelterName: string }) {
  ...
  return (
    <div className="fixed inset-0 z-[100] overflow-y-auto bg-[radial-gradient(circle_at_top,#ff5875_0%,#d92c5b_38%,#651a58_100%)] px-4 py-8 text-white">
      ...
      <button type="button" onClick={close} disabled={pending} className="absolute right-5 top-5 z-20 rounded-full bg-white/15 p-3 backdrop-blur hover:bg-white/25" aria-label="Close match"><XIcon className="size-5" /></button>
      <div className="relative z-10 mx-auto flex min-h-[calc(100svh-4rem)] max-w-2xl flex-col items-center justify-center text-center">
        <div className="mb-4 flex items-center gap-2 rounded-full border border-white/20 bg-white/12 px-4 py-2 text-sm font-semibold backdrop-blur"><SparklesIcon className="size-4" />A shelter approved your request</div>
        <h1 className="text-5xl font-black italic tracking-[-0.06em] sm:text-7xl">IT&apos;S A MATCH!</h1>
        <p className="mt-3 max-w-lg text-lg text-white/85">You and <strong className="text-white">{dogName}</strong> can take the next step. {shelterName} opened a private chat for you.</p>
        ...
        {barkAudioPath && (
          <button type="button" className="mt-6 flex items-center gap-2 rounded-full bg-white/15 px-4 py-2 text-sm font-semibold backdrop-blur hover:bg-white/25" onClick={(e) => { const audio = e.currentTarget.querySelector("audio"); audio?.play(); }}>
            <Volume2Icon className="size-4" />Hear {dogName} say hi<audio src={publicDogMediaUrl(barkAudioPath)} preload="none" />
          </button>
        )}

        <div className="mt-8 grid w-full max-w-md gap-3 sm:grid-cols-2">
          <Button size="lg" onClick={openChat} disabled={pending} className="w-full bg-white text-primary hover:bg-white/90"><MessageCircleIcon data-icon="inline-start" />Start chatting</Button>
          <Button size="lg" variant="outline" disabled={pending} onClick={close} className="border-white/40 bg-white/10 text-white hover:bg-white/20 hover:text-white"><PawPrintIcon data-icon="inline-start" />Keep discovering</Button>
        </div>
        <p className="mt-5 text-xs text-white/65">Next: chat, arrange a meet & greet, and let the shelter guide the adoption process.</p>
      </div>
    </div>
  );
}
